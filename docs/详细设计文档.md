...
2. 抓取模式 `mode`
3. 自动抓取开关 `enabled`
4. 抓取周期 `intervalMinutes`
5. 搜索条件 `queryText`（仅搜索模式）
6. 热点范围参数 `region / language`（仅热点模式）
7. 最近抓取时间 `lastRunAt`
8. 下一次执行时间 `nextRunAt`
...
3. 标签关系需记录来源 `MANUAL`、`AI` 或 `RULE`，支持人工覆盖。
...
3. 校验输出结构并落库到帖子分类字段、标签关系和 AI 任务记录。
4. 若任务失败，记录错误并允许后续重试。

#### 4.11.3 人工覆盖策略

1. 人工修改分类或标签后，将帖子标记为“手动锁定”。
2. 对于已锁定字段，后续自动 AI 任务默认不覆盖，除非用户显式允许。
