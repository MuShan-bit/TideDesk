...
5. X 推荐流抓取依赖登录态，因此系统使用可插拔的“凭证采集器 + 抓取适配器”设计，不在本设计中强绑定具体抓取实现。
...
3. 业务层：NestJS 的绑定管理、抓取调度、归档管理、任务记录、仪表盘统计模块。
...
2. 初始化抓取适配器实例。
3. 请求推荐流数据。
4. 对响应内容进行标准化，得到统一 `NormalizedPost` 列表。
5. 逐条写入 `crawl_run_posts` 处理记录。
6. 新帖子写入归档表，已存在帖子标记为 `SKIPPED`。
7. 汇总本次执行结果并更新 `crawl_runs`。
